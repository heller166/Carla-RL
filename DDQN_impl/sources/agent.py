...
# Runs agent process
def run(id, gifdir, action_val_dir, carla_instance, stop, pause, episode, epsilon, show_preview, weights, weights_iteration, transitions, tensorboard_stats, agent_stats, carla_frametimes, seconds_per_episode):
    ...
    while stop.value != STOP.stopping:
        ...
        while True:
            ...
            if show_preview[0] == 1:
                ...
                if episode.value % settings.ACTIONVAL_EVERY_X_EPISODES == 0 \
                    ...
                    qs_log = agent.get_qs(current_state)
                    actval_images.append((current_state[0], qs_log[0], step))
        ...
        if done:
            if episode.value % settings.GIF_EVERY_X_EPISODES == 0:
                imageio.mimsave(gifdir + '/ddqn_{}.gif'.format(episode.value), gif_images, fps=20)
            if episode.value % settings.ACTIONVAL_EVERY_X_EPISODES == 0:
                ...
                for image_step_info in actval_images:
                    imageio.imsave(episode_dir
                                   + '/ddpg_step_{}_f_{}_fL_{}_fR_{}_b_{}_bL_{}_bR_{}.jpg'
                                   .format(image_step_info[2],
                                           round(image_step_info[1][0], 6),
                                           round(image_step_info[1][1], 6),
                                           round(image_step_info[1][2], 6),
                                           round(image_step_info[1][3], 6),
                                           round(image_step_info[1][4], 6),
                                           round(image_step_info[1][5], 6)),
                                   image_step_info[0])
            gif_images = []
