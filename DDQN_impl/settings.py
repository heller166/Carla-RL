...
CARLA_HOSTS_NO = 1
CARLA_HOSTS = [['localhost', 2000, 'Town01']]  # List of hosts and ports and worlds to use, at least 2 ports of difference as Carla uses N and N+1 port, Town01 to Town97 for world currently, Town01 to Town07 for world are currently available, int number instead - random world change interval in minutes
...
RANDOM_SPAWN = False #If true chooses random spawn. If fasle uses first spawn
GIF_EVERY_X_EPISODES = 500
ACTIONVAL_EVERY_X_EPISODES = 200
ACTIONVAL_EVERY_X_STEPS = 20

# Agent settings
AGENTS = 1
AGENT_MEMORY_FRACTION = 0.05
...
MODEL_NAME = 'ddqn_32x3_#CNN_KERNELS#'  # model name, prefixed from sources/models.py, #MODEL_ARCHITECTURE# adds model architectore acronym, #CNN_KERNELS# adds number of kernels from all CNN layers
MIN_REWARD = 100  # For model save
TRAINER_MEMORY_FRACTION = 0.15
...
MODEL_SETTINGS = {'hidden_1_units': 200}  # 'hidden_1_units': 1024 for Xception
